...
STATUS=$EXP/status/profile_split_stages_qfactory_v20.status
OUT=$EXP/results/profile_split_stages_qfactory_v20
QFCACHE=$EXP/reports/qfactory_cache_v20
...
"$PYBIN" -u "$EXP/tools/profile_split_stages_qfactory_v20.py" \
  --model "$MODEL" \
  --layer_idx 0 \
  --seq_len 128 \
  --batches 16,64 \
