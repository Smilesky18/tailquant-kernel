...
STATUS=$EXP/status/full_qwen3_8b_layer_latency_policy_projected_v22b.status
OUT=$EXP/results/full_qwen3_8b_layer_latency_policy_projected_v22b
CONT=$EXP/results/full_qwen3_8b_layer_latency_policy_v22
COMPARE=$EXP/results/compare_v22_continuous_vs_projected_v22b

# 复用 v22 的 QFactory cache，避免同一批 K/N/M 重复 JIT 编译。
QFCACHE=${QFCACHE:-$EXP/reports/qfactory_cache_v22}

mkdir -p "$EXP/logs" "$EXP/status" "$EXP/results" "$EXP/reports" "$OUT" "$COMPARE" "$QFCACHE"
...
echo "[POLICY] $POLICY"
echo "[OUT] $OUT"
echo "[CONTINUOUS_DIR] $CONT"
echo "[COMPARE] $COMPARE"
echo "[QFCACHE] $QFCACHE"
echo "[RATIO_FIELD] ratio_projected"
...
if [ ! -f "$CONT/layer_latency_all_v22.csv" ]; then
  echo "[ERROR] v22 continuous result not found: $CONT/layer_latency_all_v22.csv"
  exit 3
fi
...
"$PYBIN" -u "$EXP/tools/bench_full_qwen3_8b_layer_latency_policy_projected_v22b.py" \
  --model "$MODEL" \
  --policy "$POLICY" \
  --seq_len 128 \
  --batches 16,64 \
...
"$PYBIN" -u "$EXP/tools/compare_v22_continuous_vs_projected_v22b.py" \
  --continuous_dir "$CONT" \
  --projected_dir "$OUT" \
  --out_dir "$COMPARE"
