...
def ensure_realpolicy_forward_float(layer):
    """
    v28d:
    pack_a_full_s4 expects Float input. Llama decoder internals may pass Half
    into patched RealPolicyLinear even if the outer hidden_states is Float.
    Force each patched W4A4 linear to cast its own input before pack.
    """
    patched = 0
    ...
    for mod in layer.modules():
        ...
        is_real_policy = (
            cls == "RealPolicyLinear"
            or "RealPolicyLinear" in cls
            or (hasattr(mod, "_pure") and hasattr(mod, "scratch_pool") and hasattr(mod, "ext"))
        )

        if not is_real_policy:
            continue

        if getattr(mod, "_v28d_forward_float_patch", False):
            continue

        old_forward = mod.forward

        def new_forward(x, *args, old_forward=old_forward, **kwargs):
            if isinstance(x, torch.Tensor) and x.dtype != torch.float32:
                x = x.float()
            return old_forward(x, *args, **kwargs)

        mod.forward = new_forward
        mod._v28d_forward_float_patch = True
        patched += 1
...
def ensure_scratch_has_pure_keys(layer, device):
    ...
    for mod in layer.modules():
        ...
        sp = mod.scratch_pool
        if getattr(sp, "_v28d_a_scale_patch", False):
            continue
...
def patch_layer(base_layer, B, main_ext, layout_ext, policy_pack_ext, policy_rows, eps, device):
    ...
    forward_patched = ensure_realpolicy_forward_float(layer)
    log(f"[FORWARD_FLOAT_PATCH] {forward_patched}")
...
def main():
    ...
    for layer_idx in layer_ids:
        ...
        write_csv(out / f"{args.label}_policy_debug_v28d.csv", policy_debug_rows)
        ...
        for batch in batches:
            ...
            h_fp16 = h_bf16.to(torch.float32)  # v28d: pack_a_full_s4 expects Float input
            ...
            write_csv(out / f"{args.label}_layer_latency_v28d.csv", rows)
            json.dump(rows, open(out / f"{args.label}_layer_latency_v28d.json", "w"), indent=2, ensure_ascii=False)
    ...
    write_csv(out / f"{args.label}_summary_v28d.csv", summary_rows)
    json.dump(summary_rows, open(out / f"{args.label}_summary_v28d.json", "w"), indent=2, ensure_ascii=False)

    log(f"[CSV] {out / f'{args.label}_policy_debug_v28d.csv'}")
    log(f"[CSV] {out / f'{args.label}_layer_latency_v28d.csv'}")
    log(f"[CSV] {out / f'{args.label}_summary_v28d.csv'}")
