...
__global__ void fused_topr_pack_cub_dyn_kernel(
  ...
  auto* sort_storage = reinterpret_cast<typename BlockSort::TempStorage*>(smem);
  unsigned char* after_sort = smem + sizeof(typename BlockSort::TempStorage);
  uint32_t* bits = reinterpret_cast<uint32_t*>(align_ptr(after_sort, alignof(uint32_t)));
...
void launch_fused_topr_pack(
    ...
    float eps) {
  using BlockSort = cub::BlockRadixSort<float, BLOCK_THREADS, ITEMS_PER_THREAD, int>;
  size_t smem = sizeof(typename BlockSort::TempStorage) + ((K + 31) / 32) * sizeof(uint32_t) + 32;
  auto kernel = fused_topr_pack_cub_dyn_kernel<K, BLOCK_THREADS, ITEMS_PER_THREAD>;
  cudaError_t attr_status = cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(smem));
...
void fused_topr_pack(
  ...
  TORCH_CHECK((K == 4096 || K == 12288), "fused_topr_pack v46 currently supports K=4096 or K=12288 only");
  TORCH_CHECK(R > 0 && R <= 256 && R < K, "fused_topr_pack v46 supports 0<R<=256");
  ...
  if (K == 4096) {
    launch_fused_topr_pack<4096, 256, 16>(A, R, descending_rank, A_pack, body_scale, top_scale, top_q, idx, M, static_cast<float>(eps64));
  } else {
    launch_fused_topr_pack<12288, 256, 48>(A, R, descending_rank, A_pack, body_scale, top_scale, top_q, idx, M, static_cast<float>(eps64));
  }
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("fused_topr_pack", &fused_topr_pack, "CUB dynamic-smem fused top-r selection and split activation packing v46");
}
"""
