...
def summarize_records(records: List[dict]) -> dict:
    ...
    return {
        ...
        "eligible_k4096_r256": sum(1 for r in records if int(r["K"]) == 4096 and 0 < int(r["R"]) <= 256),
        "eligible_k4096_r512": sum(1 for r in records if int(r["K"]) == 4096 and 0 < int(r["R"]) <= 512),
        "eligible_v57": sum(1 for r in records if int(r["K"]) in (4096, 12288) and 0 < int(r["R"]) <= 512),
    }
...
def install_threshold_topr_prepare_v57():
    def patch_threshold_topr_prepare_v57(mod: nn.Module, threshold_ext) -> bool:
        if getattr(mod, "_threshold_topr_prepare_v57", False):
            return False
        ...
        mod._threshold_topr_prepare_v57 = True
        mod._threshold_topr_idx_cache_v57 = idx_cache
        return True

    V43.patch_fused_topr_prepare = patch_threshold_topr_prepare_v57
    log("[INSTALL_THRESHOLD_TOPR_PREPARE_V57] K in {4096,12288}, R<=512")
...
def patch_gate_up_shared_hadamard(layer: nn.Module) -> List[str]:
    mlp = getattr(layer, "mlp", None)
    if mlp is None or getattr(mlp, "_gate_up_shared_hadamard_v57", False):
        return []

    gate = getattr(mlp, "gate_proj", None)
    up = getattr(mlp, "up_proj", None)
    down = getattr(mlp, "down_proj", None)
    if not all(V43.is_real_policy_linear(m) for m in (gate, up, down)):
        return []
    ...
    def shared_mlp_forward(self, hidden_states: torch.Tensor):
        ...
        rotate_flags = [bool(getattr(gate_proj, "rotate_online", False)), bool(getattr(up_proj, "rotate_online", False))]
        shared_hidden = hidden_states
        if any(rotate_flags):
            shared_hidden = gate_proj._rotate(hidden_states)
            gate_proj.rotate_online = False
            up_proj.rotate_online = False
        try:
            hidden_states = self.act_fn(gate_proj(shared_hidden)) * up_proj(shared_hidden)
        finally:
            gate_proj.rotate_online = rotate_flags[0]
            up_proj.rotate_online = rotate_flags[1]
        return self.down_proj(hidden_states)

    mlp.forward = types.MethodType(shared_mlp_forward, mlp)
    mlp._gate_up_shared_hadamard_v57 = True
    return ["mlp.forward_shared_gate_up_hadamard_v57"]


def build_split_layer(base_layer, layer_idx, policy, rot_flags, B, main_ext, layout_ext, policy_pack_ext, eps, device, qext, top_ext, use_gateup_shared: bool):
    ...
    shared = patch_gate_up_shared_hadamard(layer) if use_gateup_shared else []
    if shared:
        log("[PATCH_GATE_UP_SHARED_V57] " + json.dumps(shared))
    return layer, rec
...
def main():
    ...
    log("[NOTE] v57 benchmark: radix-threshold top-r pack supports K=4096/12288,R<=512; gate/up shared Hadamard is a separate runtime optimization.")
    ...
    log(f"[ROT_ORIGINAL] selected={sum(bool(v) for v in rot_flags.values())}/{len(rot_flags)}")

    variants = [
        ("baseline_v42", fused_v42, "v42_K4096_R256", "v42", False),
        ("fused512_v55", fused_v55, "v55_K4096_R512", "v55", False),
        ("threshold_v57", threshold_v57, "v57_threshold_K4096_12288_R512", "v57", False),
        ("threshold_v57_gateup_shared", threshold_v57, "v57_threshold_K4096_12288_R512", "v57", True),
    ]
    ...
    for batch in parse_ints(args.batches):
        ...
        for variant, top_ext, top_desc, patch_kind, use_gateup_shared in variants:
            if patch_kind == "v55":
                install_fused_topr_prepare_v55()
            elif patch_kind == "v57":
                install_threshold_topr_prepare_v57()
            else:
            ...
            row = {
                ...
                "topr_backend": top_desc,
                "gate_up_shared_hadamard": bool(use_gateup_shared),
            }
            try:
            ...
            rows.append(row)
            write_csv(out_dir / f"{args.label}_split_opts_v57_partial.csv", rows)
            json.dump(rows, open(out_dir / f"{args.label}_split_opts_v57_partial.json", "w"), indent=2)
            gc.collect()
    ...
    for batch in parse_ints(args.batches):
        base_rows = [r for r in rows if r.get("batch") == batch and r.get("variant") == "fused512_v55" and "split_ms" in r]
        if not base_rows:
            continue
        base = float(base_rows[0]["split_ms"])
        for r in rows:
            if r.get("batch") == batch and "split_ms" in r:
                r["speedup_over_fused512_v55"] = base / float(r["split_ms"]) if float(r["split_ms"]) > 0 else 0.0
                r["delta_vs_fused512_v55_ms"] = float(r["split_ms"]) - base

    csv_path = out_dir / f"{args.label}_split_opts_v57.csv"
    json_path = out_dir / f"{args.label}_split_opts_v57.json"
    meta_path = out_dir / f"{args.label}_split_opts_meta_v57.json"
