...
def main():
    ...
    log("[NOTE] total timings use CUDA Graph + events. Split dense/sparse/other is a synchronized per-stage attribution; dense/sparse/other are raw component proportions normalized to split_total; other includes top-r/pack/Hadamard/reshape/bias and residual unmodeled layer work.")
    ...
    for layer_idx in layer_indices:
        ...
        for batch in batches:
            ...
            rows.append(row)
            write_csv(out_dir / f"{args.label}_prefill_layer_latency_v48_partial.csv", rows)
            json.dump(rows, open(out_dir / f"{args.label}_prefill_layer_latency_v48_partial.json", "w"), indent=2, ensure_ascii=False)
            del hidden_fp16, hidden_bf16, position_ids, pe_fp16, pe_bf16
            gc.collect()
            torch.cuda.empty_cache()

    layer_csv = out_dir / f"{args.label}_prefill_layer_latency_v48.csv"
    layer_json = out_dir / f"{args.label}_prefill_layer_latency_v48.json"
    linear_csv = out_dir / f"{args.label}_split_component_linear_v48.csv"
    linear_json = out_dir / f"{args.label}_split_component_linear_v48.json"
    meta_path = out_dir / f"{args.label}_prefill_layer_latency_meta_v48.json"
