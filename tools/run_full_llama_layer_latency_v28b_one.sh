...
STATUS=$EXP/status/${LABEL}_full_layer_latency_v28b.status
OUT=$EXP/results/${LABEL}_full_layer_latency_v28b
QFCACHE=$EXP/reports/qfactory_cache_${LABEL}_v28b
...
"$PYBIN" -u "$EXP/tools/bench_full_llama_layer_latency_policy_v28b.py" \
  --model "$MODEL" \
  --policy "$POLICY" \
  --label "${LABEL}_smoke" \
  --out_dir "$OUT/smoke" \
