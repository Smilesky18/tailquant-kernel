...
def main():
    ...
    log("[NOTE] no GPTQ, no calibration, RTN/maxabs per-output weight scale; BF16 baseline vs FP16-shell Split latency only")
    ...
    # BF16 baseline keeps BF16.
    bf16_layer = copy.deepcopy(layers[args.layer_idx]).to(device=device, dtype=dtype).eval()

    # The real split fullstack path normalizes inference parameters to FP16 before patching.
    # Mirror that here to avoid Q/K/V dtype mismatch in attention:
    #   query/key may be promoted by RoPE, while value remains Half.
    # For latency, this is the intended low-bit inference shell: FP16 non-Linear ops + int4 Linear kernels.
    split_runtime_dtype = torch.float16
    split_layer = copy.deepcopy(layers[args.layer_idx]).to(device=device, dtype=split_runtime_dtype).eval()
    ...
    for batch in batches:
        ...
        hidden_states_bf16 = torch.randn(batch, args.seq_len, hidden_size, device=device, dtype=dtype)
        hidden_states_split = hidden_states_bf16.to(torch.float16).contiguous()

        position_ids = make_position_ids(batch, args.seq_len, device)

        position_embeddings_bf16 = build_position_embeddings(model, hidden_states_bf16, position_ids)
        if position_embeddings_bf16 is None:
            raise RuntimeError("position_embeddings_bf16 is None")
        position_embeddings_bf16 = tuple(t.to(device=device, dtype=dtype).contiguous() for t in position_embeddings_bf16)

        position_embeddings_split = build_position_embeddings(model, hidden_states_split, position_ids)
        if position_embeddings_split is None:
            raise RuntimeError("position_embeddings_split is None")
        position_embeddings_split = tuple(t.to(device=device, dtype=torch.float16).contiguous() for t in position_embeddings_split)

        log("[POSITION_EMBEDDINGS_BF16] " + str([(tuple(t.shape), str(t.dtype)) for t in position_embeddings_bf16]))
        log("[POSITION_EMBEDDINGS_SPLIT] " + str([(tuple(t.shape), str(t.dtype)) for t in position_embeddings_split]))
        ...
        if args.timing in {"eager", "both"}:
            bf16_eager_ms = bench_eager(
                bf16_layer,
                hidden_states_bf16,
                position_ids,
                position_embeddings_bf16,
                args.warmup,
                args.iters,
            )
            split_eager_ms = bench_eager(
                split_layer,
                hidden_states_split,
                position_ids,
                position_embeddings_split,
                args.warmup,
                args.iters,
            )
        ...
        if args.timing in {"graph", "both"}:
            ...
            try:
                ...
                split_graph_ms = None
                split_eager_fallback_ms = bench_eager(
                    split_layer,
                    hidden_states_split,
                    position_ids,
                    position_embeddings_split,
    ...
    csv_path = out_dir / "bf16_vs_split_no_gptq_layer_latency_v7.csv"
    json_path = out_dir / "bf16_vs_split_no_gptq_layer_latency_v7.json"
    records_path = out_dir / "split_patch_records_v7.json"
