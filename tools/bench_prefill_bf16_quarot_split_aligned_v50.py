...
def main():
    ...
    log("[NOTE] total timings use CUDA Graph + events. v50 aligns the Llama/Qwen QuaRot fallback with the original QuaRot module structure where possible. split_dense_only_ms uses RealPolicyLinear pure with normal online Hadamard; split_dense_only_no_online_rotate_ms disables online Hadamard to isolate rotation overhead. Per-stage dense/sparse/other fields remain synchronized diagnostics only.")
    ...
    for layer_idx in layer_indices:
        ...
        for batch in batches:
            ...
            rows.append(row)
            write_csv(out_dir / f"{args.label}_prefill_layer_latency_v50_partial.csv", rows)
            json.dump(rows, open(out_dir / f"{args.label}_prefill_layer_latency_v50_partial.json", "w"), indent=2, ensure_ascii=False)
            del hidden_fp16, hidden_bf16, position_ids, pe_fp16, pe_bf16
            gc.collect()
            torch.cuda.empty_cache()

    layer_csv = out_dir / f"{args.label}_prefill_layer_latency_v50.csv"
    layer_json = out_dir / f"{args.label}_prefill_layer_latency_v50.json"
    linear_csv = out_dir / f"{args.label}_split_component_linear_v50.csv"
    linear_json = out_dir / f"{args.label}_split_component_linear_v50.json"
    meta_path = out_dir / f"{args.label}_prefill_layer_latency_meta_v50.json"
