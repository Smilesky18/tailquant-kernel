...
def resolve_extensions(B, args, out_dir: Path):
    ...
    for name in dir(B):
        try:
            ...
            continue
        lname = name.lower()
        if "ext" in lname or "kernel" in lname or "layout" in lname or "policy" in lname:
            module_objs[name] = str(type(obj))
    dump_debug(out_dir, "module_ext_like_symbols_v5.json", module_objs)

    direct = normalize_ext_tuple({name: getattr(B, name) for name in dir(B) if "ext" in name.lower() or "layout" in name.lower()})
    if direct is not None:
        log("[EXT] resolved from module-level objects")
        return direct

    # 再尝试 loader/build/compile 函数。
    candidate_names = []
    for name in dir(B):
        lname = name.lower()
        if any(k in lname for k in ["extension", "extensions", "compile", "build", "load_ext", "load_kernel", "kernel_ext"]):
            obj = getattr(B, name)
            if callable(obj):
    ...
    for name in candidate_names:
        ...
        for pname, param in sig.parameters.items():
            if param.default is inspect._empty and param.kind in (param.POSITIONAL_ONLY, param.VAR_POSITIONAL):
                unsupported_required.append(pname)
                continue

            lp = pname.lower()
            if "cutlass" in lp:
                kwargs[pname] = args.cutlass_path
            elif "verbose" in lp:
                kwargs[pname] = False
            elif "out" in lp and "dir" in lp:
                kwargs[pname] = str(out_dir / "ext_build")
            elif "root" in lp:
                kwargs[pname] = str(Path.cwd())
            elif "device" in lp:
                kwargs[pname] = torch.device(args.device)

        if unsupported_required:
            attempts.append({"name": name, "signature": str(sig), "skip": f"unsupported required positional {unsupported_required}"})
            continue

        call_variants = []
        call_variants.append(("kwargs", kwargs))
        call_variants.append(("empty", {}))

        for variant, kw in call_variants:
            try:
                log(f"[EXT_TRY] {name} variant={variant} kwargs={kw}")
                ret = fn(**kw)
                norm = normalize_ext_tuple(ret)
                attempts.append({"name": name, "signature": str(sig), "variant": variant, "ok": True, "ret_type": str(type(ret))})
                if norm is not None:
                    dump_debug(out_dir, "extension_resolve_attempts_v5.json", attempts)
                    log(f"[EXT] resolved by {name} variant={variant}")
                    return norm
            except Exception as e:
                attempts.append({"name": name, "signature": str(sig), "variant": variant, "ok": False, "error": repr(e)})

    dump_debug(out_dir, "extension_resolve_attempts_v5.json", attempts)

    # 最后把 main 源码头部也 dump，方便继续手工接。
    try:
        src = inspect.getsource(B.main)
    except Exception as e:
        src = f"<cannot get B.main source: {e!r}>"
    (out_dir / "bench_real_split_main_source_v5.py.txt").write_text(src)
    log(f"[DEBUG_DUMP] {out_dir / 'bench_real_split_main_source_v5.py.txt'}")

    raise RuntimeError("Cannot resolve main_ext/layout_ext/policy_pack_ext. See extension_resolve_attempts_v5.json and bench_real_split_main_source_v5.py.txt")
