#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Unsmoothed grouped low-ratio search with RoMeO block-Hadamard fallback."""
...
def apply_offline_weight_rotation_search(model, rot_flags):
    ...
    print(
        f"[RoMeoRotateOptSplitSearchBlockHad] up_norm_scales={len(_NORM_SCALE_BY_FULL_NAME)} "
...
def register_romeo_rotation_hooks_for_layer(
    ...
    for local_name, module in linears.items():
        ...
        def make_had_hook(local_plan: HadPlan):
            def hook(_module, inputs):
                x = apply_had_plan(H, inputs[0], local_plan)
                return (x,) + inputs[1:]
            return hook
