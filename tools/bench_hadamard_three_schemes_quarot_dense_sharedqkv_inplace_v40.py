...
def build_split_layer_with_hadamard(base_layer, layer_idx, policy, rot_flags, B, main_ext, layout_ext, policy_pack_ext, eps, device, quarot_dense_ext=None):
    BASE = getattr(B, "BASE")
    ...
    if quarot_dense_ext is not None:
        patch_quarot_dense_backend(layer, quarot_dense_ext)
    shared = patch_qwen_qkv_shared_preprocess(layer)
    if shared:
        log("[PATCH_SHARED_QKV_V39] " + json.dumps(shared, indent=2))
...
def patch_quarot_dense_backend(layer: nn.Module, qext):
    import types
    ...
    log("[PATCH_QUAROT_DENSE_BACKEND] " + json.dumps(patched, indent=2))
    return patched
...
def patch_qwen_qkv_shared_preprocess(layer: nn.Module):
    attn = getattr(layer, "self_attn", None)
    if attn is None or getattr(attn, "_shared_qkv_preprocess_v39", False):
        return []

    patched_topk = patch_shared_topk_prepare_for_qkv(attn)
...
def main():
    ...
    log("[NOTE] Complete method paths are preserved; Split dense W4A4 uses QuaRot SM120 CUTLASS backend; q/k/v share Hadamard and k/v share topk; sparse correction is applied inplace to dense output.")
    ...
    for layer_idx in layer_indices:
        for batch in batches:
            ...
            row = {
                ...
                "note": "hadamard_on_complete_paths_split_quarot_dense_sharedqkv_inplace_no_gptq_latency_only",
            }
            ...
            for variant in variants:
                ...
                try:
                    if variant == "pure":
                        ...
                        layer, rec = build_split_layer_with_hadamard(layers[layer_idx], layer_idx, policy, rot_flags, B, main_ext, layout_ext, policy_pack_ext, args.eps, device, quarot_dense_ext)
    ...
    csv_path = out_dir / f"{args.label}_hadamard_three_schemes_quarot_dense_sharedqkv_inplace_v40.csv"
    json_path = out_dir / f"{args.label}_hadamard_three_schemes_quarot_dense_sharedqkv_inplace_v40.json"
