...
void fused_topr_pack(
  ...
  TORCH_CHECK((K == 4096 || K == 12288), "fused_topr_pack v44 currently supports K=4096 or K=12288 only");
  TORCH_CHECK(R > 0 && R <= 256 && R < K, "fused_topr_pack v44 supports 0<R<=256");
  ...
  if (K == 4096) {
    ...
    fused_topr_pack_cub_kernel<12288, 256, 48><<<M, 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        reinterpret_cast<const half*>(A.data_ptr<at::Half>()),
        R,
        descending_rank,
        A_pack.data_ptr<uint8_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("fused_topr_pack", &fused_topr_pack, "CUB fused top-r selection and split activation packing v44");
}
"""
