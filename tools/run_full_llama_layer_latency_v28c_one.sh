...
STATUS=$EXP/status/${LABEL}_full_layer_latency_v28c.status
OUT=$EXP/results/${LABEL}_full_layer_latency_v28c
QFCACHE=$EXP/reports/qfactory_cache_${LABEL}_v28c
...
"$PYBIN" -u "$EXP/tools/bench_full_llama_layer_latency_policy_v28c.py" \
  --model "$MODEL" \
  --policy "$POLICY" \
  --label "${LABEL}_smoke" \
  --out_dir "$OUT/smoke" \
