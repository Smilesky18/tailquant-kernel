...
def parse_args():
    ...
    p.add_argument("--variants", default="bf16,romeo,quarot,split")
...
def build_quarot_linear4bit_fallback_layer(base_layer: nn.Module, device: torch.device):
    """Build a standalone QuaRot-like Llama/Qwen layer without using the stale e2e wrapper.

    This mirrors the public QuaRot wrapper structure more closely than v48/v49:
    q/k/v share one activation Quantizer, gate/up share one activation Quantizer,
    and o_proj/down_proj keep the online Hadamard placement used by QuaRot.
    """
    import quarot
    from transformers.modeling_utils import ALL_ATTENTION_FUNCTIONS
    from transformers.models.llama.modeling_llama import eager_attention_forward as llama_eager_attention_forward
    from transformers.models.llama.modeling_llama import apply_rotary_pos_emb as llama_apply_rotary_pos_emb
    from transformers.models.qwen3.modeling_qwen3 import eager_attention_forward as qwen_eager_attention_forward
    from transformers.models.qwen3.modeling_qwen3 import apply_rotary_pos_emb as qwen_apply_rotary_pos_emb

    layer = copy.deepcopy(base_layer).to(device=device, dtype=torch.float16).eval()

    attn = getattr(layer, "self_attn", None)
    if attn is not None:
        if all(isinstance(getattr(attn, n, None), nn.Linear) for n in ("q_proj", "k_proj", "v_proj", "o_proj")):
            attn.quantizer = quarot.nn.Quantizer()
            attn.q_proj = quarot.nn.Linear4bit.from_float(attn.q_proj)
            attn.k_proj = quarot.nn.Linear4bit.from_float(attn.k_proj)
            attn.v_proj = quarot.nn.Linear4bit.from_float(attn.v_proj)
            num_heads = int(getattr(attn, "num_heads", getattr(getattr(layer, "config", None), "num_attention_heads", 1)))
            attn.o_proj_hadamard = quarot.nn.OnlineHadamard(num_heads)
            attn.o_proj = nn.Sequential(quarot.nn.Quantizer(), quarot.nn.Linear4bit.from_float(attn.o_proj))

            def quarot_attention_forward(
                self,
                ...
                **kwargs,
                ...
                hidden_q = self.quantizer(hidden_states)
                query_states = self.q_proj(hidden_q).view(hidden_shape).transpose(1, 2)
                key_states = self.k_proj(hidden_q).view(hidden_shape).transpose(1, 2)
                value_states = self.v_proj(hidden_q).view(hidden_shape).transpose(1, 2)
                cos, sin = position_embeddings
                model_type = str(getattr(getattr(self, "config", None), "model_type", "")).lower()
                if "qwen" in model_type:
                    query_states, key_states = qwen_apply_rotary_pos_emb(query_states, key_states, cos, sin)
                    eager_fn = qwen_eager_attention_forward
                else:
                    query_states, key_states = llama_apply_rotary_pos_emb(query_states, key_states, cos, sin)
                    eager_fn = llama_eager_attention_forward
            ...
            attn.forward = types.MethodType(quarot_attention_forward, attn)

    mlp = getattr(layer, "mlp", None)
    if mlp is not None:
        if all(isinstance(getattr(mlp, n, None), nn.Linear) for n in ("gate_proj", "up_proj", "down_proj")):
            mlp.quantizer = quarot.nn.Quantizer()
            mlp.gate_proj = quarot.nn.Linear4bit.from_float(mlp.gate_proj)
            mlp.up_proj = quarot.nn.Linear4bit.from_float(mlp.up_proj)
            intermediate_size = int(getattr(mlp, "intermediate_size", getattr(mlp.down_proj, "in_features")))
            mlp.down_proj = nn.Sequential(
                quarot.nn.OnlineHadamard(intermediate_size),
                quarot.nn.Quantizer(),
                quarot.nn.Linear4bit.from_float(mlp.down_proj),
            )

            def quarot_mlp_forward(self, x: torch.Tensor):
                xq = self.quantizer(x)
                return self.down_proj(self.act_fn(self.gate_proj(xq)) * self.up_proj(xq))

            mlp.forward = types.MethodType(quarot_mlp_forward, mlp)
...
def patch_romeo_quarot_dense_backend(layer: nn.Module, qext) -> List[str]:
    """Patch RoMeo OptMixedQLinear so only its normal a4w4 body GEMM uses QuaRot.

    RoMeo's activation/weight outlier selection, a8w8/a4w8/a8w4 kernels, and
    post-reorder stay on the original path. This makes the dense int4 Tensor Core
    body comparable with QuaRot/Split without editing RoMeo source files.
    """
    import math
    import quarot
...
def main():
    ...
    log("[NOTE] total timings use CUDA Graph + events. v51 compares BF16/RoMeo/QuaRot/Split. RoMeo keeps its mixed-precision outlier path but patches the normal a4w4 dense body GEMM to QuaRot matmul; Split dense also uses QuaRot matmul. split_dense_only_no_online_rotate_ms isolates dense W4A4 without split online Hadamard.")
    ...
    for layer_idx in layer_indices:
        ...
        for batch in batches:
            ...
            try:
                ...
                if "quarot" in variants:
                    if qlayers is not None:
                        ...
                        qpe = pe_fp16
                        row["quarot_impl"] = "linear4bit_fallback_v50_aligned"
                ...
                if "romeo" in variants:
                    ...
                    row["romeo_impl"] = "romeo_mixed_outliers_quarot_a4w4_body_v51"
                    log(f"[ROMEO_QUAROT_DENSE_PATCHED] layer={layer_idx} batch={batch} modules={patched_romeo}")
                    ...
                    log(f"[TIME] layer={layer_idx} batch={batch} romeo_ms={row['romeo_ms']:.6f} impl={row['romeo_impl']}")
                    romeo_layer.to("cpu")
                    del romeo_layer
                    torch.cuda.empty_cache()
                ...
                if "split" in variants:
                    ...
                    no_rotate_flags = {k: False for k in rot_flags}
                    dense_only_no_rot_layer, pure_no_rot_rec = build_split_layer(base_layer, layer_idx, pure_policy, no_rotate_flags, B, main_ext, layout_ext, policy_pack_ext, args.eps, device, qext, None, qwen_shared)
                    row.update({f"split_dense_only_no_online_rotate_{k}": v for k, v in summarize_records(pure_no_rot_rec).items()})
                    row["split_dense_only_no_online_rotate_ms"] = V43.bench_graph(lambda: V43.run_layer_once(dense_only_no_rot_layer, hidden_fp16, position_ids, pe_fp16), args.warmup, args.iters, device)
                    log(f"[TIME] layer={layer_idx} batch={batch} split_dense_only_no_online_rotate_ms={row['split_dense_only_no_online_rotate_ms']:.6f}")
                    del dense_only_no_rot_layer
                    torch.cuda.empty_cache()
                ...
                if row.get("quarot_ms") and row.get("romeo_ms"):
                    row["romeo_over_quarot"] = row["romeo_ms"] / row["quarot_ms"]
                ...
                if row.get("split_dense_only_no_online_rotate_ms") and row.get("split_dense_only_ms"):
                    row["split_online_rotate_overhead_ms"] = row["split_dense_only_ms"] - row["split_dense_only_no_online_rotate_ms"]
                    row["split_dense_only_over_no_online_rotate"] = row["split_dense_only_ms"] / row["split_dense_only_no_online_rotate_ms"]
                if row.get("quarot_ms") and row.get("split_dense_only_no_online_rotate_ms"):
                    row["split_dense_only_no_online_rotate_over_quarot"] = row["split_dense_only_no_online_rotate_ms"] / row["quarot_ms"]
            ...
            rows.append(row)
            write_csv(out_dir / f"{args.label}_prefill_layer_latency_v51_partial.csv", rows)
            json.dump(rows, open(out_dir / f"{args.label}_prefill_layer_latency_v51_partial.json", "w"), indent=2, ensure_ascii=False)
            del hidden_fp16, hidden_bf16, position_ids, pe_fp16, pe_bf16
            gc.collect()
            torch.cuda.empty_cache()

    layer_csv = out_dir / f"{args.label}_prefill_layer_latency_v51.csv"
    layer_json = out_dir / f"{args.label}_prefill_layer_latency_v51.json"
    linear_csv = out_dir / f"{args.label}_split_component_linear_v51.csv"
    linear_json = out_dir / f"{args.label}_split_component_linear_v51.json"
    meta_path = out_dir / f"{args.label}_prefill_layer_latency_meta_v51.json"
    ...
    summary = {
        ...
        "sum_split_dense_only_no_online_rotate_ms": sum(float(r.get("split_dense_only_no_online_rotate_ms", 0.0)) for r in rows),
        "sum_split_online_rotate_overhead_ms": sum(float(r.get("split_online_rotate_overhead_ms", 0.0)) for r in rows),
