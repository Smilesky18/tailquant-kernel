...
STATUS=$EXP/status/full_qwen3_8b_layer_latency_policy_v22.status
OUT=$EXP/results/full_qwen3_8b_layer_latency_policy_v22
QFCACHE=$EXP/reports/qfactory_cache_v22
...
"$PYBIN" -u "$EXP/tools/bench_full_qwen3_8b_layer_latency_policy_v22.py" \
  --model "$MODEL" \
  --policy "$POLICY" \
  --seq_len 128 \
  --batches 16,64 \
