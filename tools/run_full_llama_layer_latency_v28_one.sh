...
STATUS=$EXP/status/${LABEL}_full_layer_latency_v28.status
OUT=$EXP/results/${LABEL}_full_layer_latency_v28
QFCACHE=$EXP/reports/qfactory_cache_${LABEL}_v28
...
"$PYBIN" -u "$EXP/tools/bench_full_llama_layer_latency_policy_v28.py" \
  --model "$MODEL" \
  --policy "$POLICY" \
  --label "$LABEL" \
  --out_dir "$OUT" \
