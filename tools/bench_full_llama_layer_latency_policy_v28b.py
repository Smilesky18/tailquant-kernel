...
def ensure_scratch_has_pure_keys(layer, device):
    ...
    for mod in layer.modules():
        ...
        sp = mod.scratch_pool
        if getattr(sp, "_v28b_a_scale_patch", False):
            continue
...
def main():
    ...
    for layer_idx in layer_ids:
        ...
        write_csv(out / f"{args.label}_policy_debug_v28b.csv", policy_debug_rows)
        ...
        for batch in batches:
            ...
            write_csv(out / f"{args.label}_layer_latency_v28b.csv", rows)
            json.dump(rows, open(out / f"{args.label}_layer_latency_v28b.json", "w"), indent=2, ensure_ascii=False)
    ...
    write_csv(out / f"{args.label}_summary_v28b.csv", summary_rows)
    json.dump(summary_rows, open(out / f"{args.label}_summary_v28b.json", "w"), indent=2, ensure_ascii=False)

    log(f"[CSV] {out / f'{args.label}_policy_debug_v28b.csv'}")
    log(f"[CSV] {out / f'{args.label}_layer_latency_v28b.csv'}")
    log(f"[CSV] {out / f'{args.label}_summary_v28b.csv'}")
