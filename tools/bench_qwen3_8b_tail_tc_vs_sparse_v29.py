...
def get_scratch(linear, M, K, N, device):
    sp = get_linear_attr(linear, ["scratch_pool"], required=False)
    if sp is not None and hasattr(sp, "get"):
        scratch = sp.get(M, K, N)
    else:
        scratch = {}
    return ensure_scratch(linear, scratch, M, K, N, device)
...
def linear_pure(linear, A, device):
    ...
    if N <= 0:
        # fallback: infer from B_col if possible
...
def main():
                ...
                for ratio_name, ratio in parse_sweep_ratios(args.sweep_ratios, policy_ratio):
                    try:
                        ...
                        if row is not None:
                            ...
                            write_csv(out / "qwen3_8b_tail_tc_vs_sparse_v29.csv", rows)
                            json.dump(rows, open(out / "qwen3_8b_tail_tc_vs_sparse_v29.json", "w"), indent=2, ensure_ascii=False)
                            write_csv(out / "qwen3_8b_tail_tc_vs_sparse_summary_v29.csv", summarize(rows))
                        ...
                        write_csv(out / "qwen3_8b_tail_tc_vs_sparse_failures_v29.csv", failures)
    ...
    write_csv(out / "qwen3_8b_tail_tc_vs_sparse_v29.csv", rows)
    write_csv(out / "qwen3_8b_tail_tc_vs_sparse_summary_v29.csv", summary)
    json.dump(rows, open(out / "qwen3_8b_tail_tc_vs_sparse_v29.json", "w"), indent=2, ensure_ascii=False)
    json.dump(summary, open(out / "qwen3_8b_tail_tc_vs_sparse_summary_v29.json", "w"), indent=2, ensure_ascii=False)

    log(f"[CSV] {out / 'qwen3_8b_tail_tc_vs_sparse_v29.csv'}")
    log(f"[CSV] {out / 'qwen3_8b_tail_tc_vs_sparse_summary_v29.csv'}")
