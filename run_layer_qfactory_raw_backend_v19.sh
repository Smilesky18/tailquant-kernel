...
STATUS=$EXP/status/layer_qfactory_raw_backend_v19.status
OUT=$EXP/results/layer_qfactory_raw_backend_v19
QFCACHE=$EXP/reports/qfactory_cache_v19
...
"$PYBIN" -u "$EXP/tools/bench_layer_qfactory_raw_backend_v19.py" \
  --model "$MODEL" \
  --layer_idx 0 \
  --seq_len 128 \
  --batches 16,64 \
